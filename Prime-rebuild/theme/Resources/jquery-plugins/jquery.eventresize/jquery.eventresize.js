/**
 *
 * Ben Alman jQuery Resize Events
 * http://benalman.com/news/2010/03/jquery-special-events/#setup-and-teardown-resize-minimal
 *
...
!(function($){
  ...
  $.event.special.csEventResize = {
    setup: function() {
      ...
      elem.data( 'csEventResize', { w: elem.width(), h: elem.height() } );
  ...
  // As long as a "csEventResize" event is bound, this function will execute
  // repeatedly.
  function poll() {
    ...
    elems.each(function(){
      var elem = $(this),
        width = elem.width(),
        height = elem.height(),
        data = elem.data( 'csEventResize' );

      // If element size has changed since the last time, update the element
      // data store and trigger the "csEventResize" event.
      if ( width !== data.w || height !== data.h ) {
        data.w = width;
        data.h = height;
        elem.triggerHandler( 'csEventResize' );
      }
    });
