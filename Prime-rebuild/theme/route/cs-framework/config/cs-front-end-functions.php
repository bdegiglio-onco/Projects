...
if( ! function_exists( 'cs_top_bar' ) ) {
  function cs_top_bar() {
    ...
    if ( cs_get_option( 'top_bar' ) && empty( $options['disable_top_bar'] ) && empty( $options['header_transparent'] ) && ( cs_get_option( 'top_left' ) || cs_get_option( 'top_right' ) ) ) {
      $out .= '<div id="top-bar">';
    ...
    if( ! empty( $modules ) ) {
      ...
      foreach ( $modules as $key => $module ) {
        ...
        switch ( $module['module'] ) {
          ...
          // ----------------------------------------------
          case 'link':
            $out  .= '<a href="'. $module['link'] .'" target="'. $module['target'] .'">';
            $out  .= ( ! empty( $module['icon'] ) ) ? '<i class="'. cs_icon_class( $module['icon'] ) .'"></i>' : '';
            $out  .= cs_multilang_value( $module['text'] );
            $out  .= '</a>';
          break;
          ...
          // ----------------------------------------------
          case 'social':
            $out  .= '<a href="'. $module['link'] .'" target="'. $module['target'] .'" class="'. cs_icon_class( $module['icon'] ) .'"></a>';
          break;
          ...
          case 'woologin':
            ...
            if( is_woocommerce_activated() ) {
              ...
              if ( is_user_logged_in() ) {
                ...
                $out  .= ' / ';
                $out  .= '<a href="'. wp_logout_url( get_permalink( wc_get_page_id( 'myaccount' ) ) ) .'"><i class="cs-in fa fa-power-off"></i>'. __( 'Logout', 'route' ) . '</a>';
              } else {
          ...
          case 'woominicart':
            ...
            if( is_woocommerce_activated() ) {
              ...
              $out  .= '<div class="cs-top-modal">';
              $out  .= '<a href="#" class="cs-open-modal"><i class="cs-in fa fa-shopping-cart"></i>'. __( 'Cart', 'route' ) . '<span class="cs-cart-count">'. WC()->cart->cart_contents_count .'</span></a>';
              $out  .= '<div class="cs-modal-content"><div class="cs-mini-cart">'. $mini_cart .'</div></div>';
              $out  .= '</div>';
          ...
          case 'wpml':
            ...
            if ( is_wpml_activated() ) {
              ...
              if ( ! empty( $icl_get_languages ) ) {
                ...
                foreach ( $icl_get_languages as $id => $current_lang ) {
                  if ( $current_lang['active'] ) {
                    $out  .= '<i class="cs-in fa fa-globe"></i>';
              ...
              $out  .= __( 'WPML is not activated', 'rotue' );
            }
          ...
          case 'qtranslate':
            ...
            if( is_qtranslate_activated() ) {
              ...
              if( ! empty( $q_languages ) ) {
                ...
                // current language
                $out  .= '<a href="#" class="cs-open-modal">';
                $out  .= '<img src="'. $q_image . $q_config['flag'][$q_current] .'" alt="'. $q_current .'" />'. $q_config['language_name'][$q_current];
                $out  .= '</a>';
                ...
                $out  .= __( 'qTranslate is not activated', 'rotue' );
              }
...
if ( ! function_exists( 'cs_posted_on' ) ) {
  function cs_posted_on() {
    ...
    if ( in_array( 'category', get_object_taxonomies( get_post_type() ) ) && route_categorized_blog() ) {
      echo '<span class="entry-cat-links">'. get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'route' ) ) .'</span>';
    }
...
if ( ! function_exists( 'cs_post_nav' ) ) {
  function cs_post_nav() {
    ...
    if ( empty( $next ) && empty( $previous ) ) { return; }
    ?>
...
if( ! function_exists('cs_portfolio_item') ) {
  function cs_portfolio_item( $item_args ) {
    ...
    echo '<div class="isotope-item cs-fade '. cs_get_bootstrap( $columns ) . $item_cats .'">';
    ...
    echo '<div class="portfolio-item-info">';
      ...
      switch ( $model ) {
        case 'ajax':
          ...
          echo '<span class="portfolio-item-categories">'.  $item_slugs .'</span>';
          echo '</span>';
          echo '</a>';
        break;
        ...
        case 'gallery':
          ...
          echo '<span class="portfolio-item-title">'. get_the_title() .'</span>';
          echo ( !empty( $item_slugs ) ) ? '<span class="portfolio-item-categories">'.  $item_slugs .'</span>' : '';
          echo '</span>';
          echo '</a>';
        break;
...
if( ! function_exists('cs_link_pages') ) {
  function cs_link_pages( $args = '' ) {
    ...
    $defaults = array(
      ...
      'separator'         => '',
      'nextpagelink'      => __( 'Next page', 'route' ),
      'previouspagelink'  => __( 'Previous page', 'route' ),
      'pagelink'          => '%',
      'echo'              => 1
    );
...
if ( ! function_exists( 'cs_site_logo' ) ) {
  function cs_site_logo( $class = '' ) {
    ...
    if( $header_style == 'default' && ! empty( $post_meta['header_transparent'] ) ) {
      ...
      $logo1x     = cs_get_option( 'logo_alt' );
      $logo2x_alt = cs_get_option( 'logo2x_alt' );

      if ( $logo1x ) {
        $logo_alt = ' data-alternative="'. $logo .'"';
        $logo     = $logo1x;
      }

      if ( $logo2x ) {
        $logo2x_alt = ' data-alternative="'. $logo2x_alt .'"';
        $logo2x     = $logo2x;
      }
    ...
    $output .= ( $logo )   ? '<img class="cs-logo" src="'. $logo .'" alt="'. get_bloginfo( 'name' ). '"'. $logo_alt .'/>' : '';
    $output .= ( $logo2x ) ? '<img class="cs-logo2x" src="'. $logo2x .'" alt="'. get_bloginfo( 'name' ). '"'. $logo2x_alt .'/>' : '';
...
if ( ! function_exists( 'cs_site_menu' ) ) {
  function cs_site_menu() {
    ...
    if ( has_nav_menu( 'primary' ) ) {
      ...
      echo '<a href="'. admin_url('nav-menus.php') .'" class="cs-sticky-item">'. __('Please assign a menu from Appearance &gt; Menus', 'route') .'</a>';
    }
    $output .= ob_get_clean();
    $output .= '</nav>';
