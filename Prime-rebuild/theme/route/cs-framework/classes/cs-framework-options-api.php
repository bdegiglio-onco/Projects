...
abstract class CSFramework_Options_API extends CSFramework_Abstract {
  ...
  public function checked( $helper = '', $current = '', $type = 'checked', $echo = false ) {
    ...
    if ( is_array( $helper ) && in_array( $current, $helper ) && ! empty( $current ) ) {
      
      $result = " $type='$type'";

    } else if ( (string) $helper === (string) $current && ! empty( $current ) ) {
...
foreach ( glob( FRAMEWORK_DIR . '/fields/*/field_*.php', GLOB_NOSORT ) as $field_key => $field_class ) {
