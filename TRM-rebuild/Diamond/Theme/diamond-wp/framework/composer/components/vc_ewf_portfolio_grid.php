<?php
	...
	#
	add_image_size( 'ewf-portfolio-grid', 380, 235, true);
	...
	function ewf_vc_portfolio_grid( $atts, $content ) {
	   ...
	   extract($options);
			...
			$_ewf_rowItems++;
			  ...
			  echo '<div class="ewf-span'.$_ewf_span.'">
					<div class="portfolio-item">
                        ...
                        <div class="portfolio-item-overlay">
							...
							<div class="portfolio-item-overlay-actions">
								<a class="magnificPopup-gallery portfolio-item-zoom" href="'.$image_preview_large[0].'" title="zoom">
									<i class="fa fa-search-plus"></i>
								</a>
								
								<a class="portfolio-item-link" href="'.get_permalink().'" title="zoom">
					...
					<div class="portfolio-item-description">
						...
						<h4 class="text-uppercase text-center">'.get_the_title().'</h4>
						<p class="text-center">'.get_the_excerpt().'</p>

					</div><!-- end .portfolio-item-descriptio -->
		...
		switch($nav){
			...
			case 'right':
				echo '<div class="ewf-row text-right">';
					echo ewf_sc_grid_navigation_pages(5, $wp_query_portfolio_grid);
				echo '</div>';
				break;
			
			case 'center':
				echo '<div class="ewf-row text-center">';
					echo ewf_sc_grid_navigation_pages(5, $wp_query_portfolio_grid);
				echo '</div>';
				break;
		}
		...
		vc_map( array(
		   ...
		   "params" => array(
			  ...
			  array(
				 ...
				 "holder" => "div",
				 "class" => "",
				 "heading" => __("Navigation", EWF_SETUP_THEME_DOMAIN),
				 "param_name" => "nav",
				 "value" => array( 
					__("Don't show navigation", EWF_SETUP_THEME_DOMAIN) 	=> 'none', 
					__('Show navigation on left', EWF_SETUP_THEME_DOMAIN)	=>'left', 
					__('Show navigation on right', EWF_SETUP_THEME_DOMAIN)	=>'right', 
					__('Show navigation on center', EWF_SETUP_THEME_DOMAIN)	=>'center', 
					),
			  ),
			  array(
				 "type" => "dropdown",
